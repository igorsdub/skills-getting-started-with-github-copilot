document.addEventListener("DOMContentLoaded", () => {
  ...
  async function updateParticipant(activity, email, method) {
    try {
      ...
      if (response.ok) {
        ...
        setMessage("error", result.detail || "An error occurred");
      }
    } catch (error) {
      setMessage("error", `Failed to ${method === "POST" ? "sign up" : "remove participant"}. Please try again.`);
    ...
    try {
      ...
      activitiesList.querySelectorAll(".participant-remove-button").forEach((button) => {
        button.addEventListener("click", () => {
          updateParticipant(button.dataset.activity, button.dataset.email, "DELETE");
        });
      });
    } catch (error) {
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      setMessage("error", "Failed to sign up. Please try again.");
